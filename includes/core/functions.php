...
if ( ! defined( 'SUPC_FUTURE_INTELLIGENCE_VERSION' ) ) {
	define( 'SUPC_FUTURE_INTELLIGENCE_VERSION', '1.1.0' );
}
...
$future_second_hardening = __DIR__ . '/class-future-intelligence-second-eighty-hardening.php';
$future_runtime          = __DIR__ . '/class-future-intelligence-runtime.php';
if (
	is_readable( $future_contract ) &&
	is_readable( $future_rest ) &&
	is_readable( $future_second_hardening ) &&
	is_readable( $future_runtime )
) {
	require_once $future_contract;
	require_once $future_rest;
	require_once $future_second_hardening;
	require_once $future_runtime;

	$second_hardening_class = 'Sabri\\UniversalComposer\\Core\\Future_Intelligence_Second_Eighty_Hardening';
	if ( class_exists( $second_hardening_class ) ) {
		$second_hardening = new $second_hardening_class();
		if ( is_callable( array( $second_hardening, 'register' ) ) ) {
			$second_hardening->register();
		}
	}
